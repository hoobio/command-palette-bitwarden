...
#pragma warning restore VSTHRD003
  ...
  // Forwarders to the shared, process-agnostic parser (Models/BitwardenItemParser.cs). Kept so the
  // existing call sites and tests refer to BitwardenCliService while the logic is shared with the
  // companion.
  internal static List<BitwardenItem> ParseItems(string json) => BitwardenItemParser.ParseItems(json);
  internal static Dictionary<string, CustomField> ParseCustomFields(JsonNode? fields) => BitwardenItemParser.ParseCustomFields(fields);
  internal static Dictionary<string, string> ParseFolders(string json) => BitwardenItemParser.ParseFolders(json);
  internal static Dictionary<string, string> ParseOrganizations(string json) => BitwardenItemParser.ParseOrganizations(json);
}
